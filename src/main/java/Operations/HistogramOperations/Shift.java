...
public class Shift extends OperationFunction {

    @Override
    public Result apply(ImageMatrix originalImage, LinkedHashMap<String, Object> params) throws Exception {
        // Se espera un parámetro "shift" de tipo Double
        double shift = (Double) params.get("shift");
        ...
        // Aplicar desplazamiento a cada canal
        for (int i = 0; i < height; i++){
            for (int j = 0; j < width; j++){
                for (int c = 0; c < 3; c++){
                    double value = matrix[i][j][c] + shift;
                    // Limitar entre 0 y 1
